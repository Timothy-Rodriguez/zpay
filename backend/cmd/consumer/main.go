...
func main() {
	...
	if err != nil {
		logger.Fatal("Failed to initialize email client", zap.Error(err))
	}
	...
	go func() {
		...
		if len(processedRecords) > 0 {
			if err := kafkaClient.MarkRecordsProcessed(ctx, processedRecords); err != nil {
				logger.Error("failed to mark records processed", zap.Error(err))
			} else {
				logger.Info("Marked records processed and committed offset", zap.Int("count", len(processedRecords)))
			}
		}

		if err != nil {
			logger.Error("failed to consume from Kafka", zap.Error(err))
		}
	}()
